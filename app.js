class SortingVisualizer {
    constructor() {
        ...
        // ---- Audio (piano note) setup ----
        this.audioCtx = null;
        this.audioEnabled = true;
        this.valueToFreq = {};           // maps an array value -> frequency (Hz)
        this.noteLetters = ['C', 'D', 'E', 'F', 'G', 'A', 'B'];
        this.semitoneFromC = { C: 0, D: 2, E: 4, F: 5, G: 7, A: 9, B: 11 };
        this.whiteNotes = this.buildWhiteNoteTable();
        this.middleIndex = this.whiteNotes.findIndex(note => note.name === 'C4');
    ...
    bindEvents() {
        ...
        if (soundToggleBtn) {
            soundToggleBtn.addEventListener('click', () => {
                ...
                soundToggleBtn.textContent = this.audioEnabled ? '🔊 Sound: On' : '🔇 Sound: Off';
                if (this.audioEnabled) this.ensureAudioContext();
            });
        }
    }
    ...
    // Builds an extended table of white-key piano notes (letters A-G, no
    // sharps/flats) with real piano frequencies, spanning well beyond a
    // standard 88-key piano in both directions so we always have enough
    // notes for array sizes up to 100 centered on Middle C (C4).
    buildWhiteNoteTable() {
        const notes = [];
        for (let octave = -4; octave <= 14; octave++) {
            for (const letter of this.noteLetters) {
                const midi = 12 * (octave + 1) + this.semitoneFromC[letter];
                const freq = 440 * Math.pow(2, (midi - 69) / 12);
                notes.push({ name: `${letter}${octave}`, midi, freq });
            }
        }
        return notes; // already ascending in pitch
    }

    // Returns N consecutive white notes centered on Middle C (C4).
    getMiddleNotes(n) {
        const total = this.whiteNotes.length;
        let start = this.middleIndex - Math.floor(n / 2);
        start = Math.max(0, Math.min(start, total - n));
        return this.whiteNotes.slice(start, start + n);
    }

    // Maps every value currently in the array to a note frequency based on
    // its rank (smallest value -> lowest note, largest value -> highest
    // note). This mapping is fixed once per array generation and stays
    // valid throughout a sort, since swaps only move values around, they
    // never change the underlying set of values.
    computeNoteMapping() {
        const n = this.array.length;
        const notes = this.getMiddleNotes(n);
        const sortedValues = [...this.array].sort((a, b) => a - b);
        this.valueToFreq = {};
        sortedValues.forEach((v, i) => {
            this.valueToFreq[v] = notes[i].freq;
        });
    }
    ...
    // Step delay (ms) driven by the speed slider - reused for both the
    // visual step pacing and the note duration, so audio speeds up/slows
    // down together with the animation.
    getStepDelayMs() {
        const speed = 11 - this.animationSpeed;
        return Math.max(10, speed * 15);
    }

    getNoteDuration() {
        const stepMs = this.getStepDelayMs();
        return Math.max(0.05, Math.min(0.4, (stepMs / 1000) * 0.85));
    }

    playNote(value, offsetSeconds = 0) {
        if (!this.audioEnabled || !this.audioCtx || value === undefined) return;
    ...
    // Plays a note for each given array index (current value at that
    // index), gently staggered so simultaneous compares/swaps sound like a
    // quick arpeggio rather than a single flat chord.
    playNotesForIndices(indices) {
        if (!this.audioEnabled) return;
        indices.forEach((idx, i) => {
            if (idx >= 0 && idx < this.array.length) {
                this.playNote(this.array[idx], i * 0.03);
            }
        });
    }
    ...
    async delay() {
        const delayTime = this.getStepDelayMs();
    ...
    async highlightBars(indices, className, updateStats = true) {
        ...
        // Play a note for every index that's being accessed (compared,
        // swapped, or used as a pivot) right as it's highlighted.
        this.playNotesForIndices(indices);
    ...
    async markAllAsSorted() {
        for (let i = 0; i < this.array.length; i++) {
            ...
            // Sweep left-to-right playing each value's note. Since the
            // array is now fully sorted ascending, this naturally plays
            // an ascending run up the white keys.
            this.playNote(this.array[i]);
